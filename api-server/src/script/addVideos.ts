...
export const createBulkVideos = expressAsyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const videosByType = {
      MAGIC_STUDIO_VIDEO: createdVideos.filter(v => v.type === VideoType.MAGIC_STUDIO_VIDEO).length,
      SYNC_STUDIO_VIDEO: createdVideos.filter(v => v.type === VideoType.SYNC_STUDIO_VIDEO).length
    };
    ...
    console.log("Error creating bulk videos:", error);
