...
export const ProgressVideoCard = ({ jobId, progress, stage, isConnecting = false }: ProgressVideoCardProps) => {
  ...
  return (
    <article
      key={jobId}
      className="relative overflow-hidden rounded-2xl border border-zinc-200/80 bg-white p-5 shadow-sm transition-all duration-300 hover:shadow-lg  "
    >
      ...
      <div className="relative mb-4 flex items-start justify-between gap-3">
        ...
        {isConnecting ? (
          <span className="inline-flex items-center gap-2 rounded-full border border-blue-200 bg-blue-50 px-2.5 py-1 text-xs font-semibold uppercase tracking-wide text-blue-700   ">
            <Loader size={14} />
            connecting
          </span>
        ) : (
      ...
      <div className="relative mb-4 rounded-xl border border-zinc-200/80 bg-zinc-50 p-4  /60">
        ...
        <div className="relative flex items-center justify-between text-sm">
          <span className="font-medium text-zinc-700 ">Current stage</span>
          <span className="rounded-md bg-zinc-200 px-2 py-1 text-xs font-semibold capitalize text-zinc-700  ">
      ...
      <p className="mt-3 text-xs text-zinc-500 ">
        Job ID: <span className="font-mono text-[11px] text-zinc-700 ">{jobId}</span>
      </p>
